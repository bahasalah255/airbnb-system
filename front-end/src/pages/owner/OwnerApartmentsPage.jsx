...
export default function OwnerApartmentsPage() {
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title="My Apartments"
        description="Manage listings, pricing and availability status with confidence."
        actions={
          <>
            <button
              type="button"
      ...
      {loading && data && <p className="text-xs text-slate-500">Refreshing apartments...</p>}
      ...
      {apartments.length === 0 ? (
        <EmptyState title="No apartments yet" description="Create your first listing to start receiving bookings." />
      ) : viewMode === 'cards' ? (
          ...
          {apartments.map((apartment) => (
            <article key={apartment.id} className="overflow-hidden rounded-[1.5rem] border border-slate-200 bg-white shadow-[0_16px_40px_-32px_rgba(15,23,42,0.5)]">
              ...
              <div className="grid grid-cols-3 gap-2 border-t border-slate-100 p-3">
                {getApartmentGallery(apartment.photos).slice(0, 3).map((photo, index) => (
                  <ApartmentImage
                    key={`${apartment.id}-${index}`}
                    photos={[photo]}
                    alt={`${apartment.name} ${index + 1}`}
                    className="h-14 w-full"
                    imageClassName="h-14 w-full rounded-xl object-cover"
                  />
                ))}
              </div>
              ...
              <p className="mt-3 text-sm font-semibold text-slate-900">{formatCurrency(apartment.price_per_night)} / night</p>
              <p className="mt-1 text-xs text-slate-500">Capacity: {apartment.capacity}</p>
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                <button
                  ...
                  className="rounded-lg border border-slate-200 px-2 py-1 text-xs"
                >
                  {apartment.is_active ? 'Deactivate' : 'Activate'}
                </button>
                <button
                  type="button"
        ...
        <>
          <AdminTable headers={['Photo', 'Name', 'Price', 'Capacity', 'Status', 'Actions']}>
            {apartments.map((apartment) => (
              <tr key={apartment.id}>
